...
def make_embedding_fn(api_key: Optional[str] = None):
    ...
    if OPENAI_EMBEDDINGS_AVAILABLE and (api_key or os.getenv("OPENAI_API_KEY")):
        embedder = OpenAIEmbeddings(openai_api_key=api_key or os.getenv("OPENAI_API_KEY"))
...
# ----------------------------
class RAGEngine:
    def __init__(self, api_key: Optional[str] = None, persist_directory: str = "./faiss_index"):
        """
    ...
    # ----------------------------
    def query(self, question: str, llm_model: str = "gpt-4o-mini") -> Tuple[str, bool, Optional[str], Optional[str]]:
        """
        ...
        if OPENAI_CLIENT_AVAILABLE and self.api_key:
            try:
                ...
                completion = client.responses.create(model=llm_model, input=assembled_prompt)
                ...
                try:
                    ...
                    if isinstance(out, list) and len(out) > 0:
                        ...
                        cnt = getattr(first, "content", None) or first.get("content", None) if isinstance(first, dict) else None
                        ...
                        if answer_text is None:
                            ...
                            answer_text = getattr(first, "text", None) or first.get("text", None) if isinstance(first, dict) else None
                ...
                if answer_text:
                    ...
                    return assembled_prompt, True, assembled_prompt, "Could not parse LLM output"
